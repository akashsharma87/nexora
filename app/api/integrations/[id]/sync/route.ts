...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!connection.tabName) {
      ...
      for (const tab of tabs) {
        const summary = await syncTab(
          connection, tab, `${connection.id}_${tab}`, true, session.user.id, property, managerName, results
        )
        tabSummaries.push(summary)
      }
    } else {
      const summary = await syncTab(
        connection, connection.tabName, connection.id, false, session.user.id, property, managerName, results
      )
      tabSummaries.push(summary)
    }
  ...
  return NextResponse.json({ ...results, tabs: tabSummaries })
}
